# Documentação (índice)

Este índice organiza a documentação do repositório por seções, facilitando a navegação.

**Seções**

- **Intro**
  - [docs/intro/introduct.md](docs/intro/introduct.md)
  - [docs/intro/configurations.md](docs/intro/configurations.md)
  - [docs/intro/learning-levels.md](docs/intro/learning-levels.md)

- **Basic Computer**
  - [docs/basicComputer/computerBasics.md](docs/basicComputer/computerBasics.md)
  - [docs/basicComputer/basicOperationSystem.md](docs/basicComputer/basicOperationSystem.md)
  - [docs/basicComputer/booleanLogic.md](docs/basicComputer/booleanLogic.md)
  - [docs/basicComputer/programmingLogic-C.md](docs/basicComputer/programmingLogic-C.md)
  - [docs/basicComputer/structurePrograms.md](docs/basicComputer/structurePrograms.md)
  - [docs/basicComputer/importaNotes.md](docs/basicComputer/importaNotes.md)

- **Notas**
  - [docs/notes/dataTypes.md](docs/notes/dataTypes.md)
  - [docs/notes/basicOperationsMath.md](docs/notes/basicOperationsMath.md)
  - [docs/notes/compilers.md](docs/notes/compilers.md)

- **Guia completo**
  - [docs/guia-completo-c.md](docs/guia-completo-c.md)

- **Exemplos por nível**
  - [docs/examples-c/README.md](docs/examples-c/README.md)

- **Referências**
  - [docs/references/importantBooks.md](docs/references/importantBooks.md)
  - [docs/references/advancedTips.md](docs/references/advancedTips.md)
  - [docs/references/rulesGold.md](docs/references/rulesGold.md)
  - [docs/references/tips.md](docs/references/tips.md)
  - [docs/references/calculator-project-cLab.md](docs/references/calculator-project-cLab.md)

- **Imagens**
  - [docs/images/computerArchitcture.png](docs/images/computerArchitcture.png)

Uso

- Navegue até a pasta correspondente para ver exercícios e exemplos.
- A pasta [projects](../projects/) fica reservada para projetos práticos e aplicações.
- O guia completo está disponível em [docs/guia-completo-c.md](docs/guia-completo-c.md).
